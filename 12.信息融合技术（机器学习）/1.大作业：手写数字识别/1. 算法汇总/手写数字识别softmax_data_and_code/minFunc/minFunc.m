function [x,f,cost_all,exitflag,output] = minFunc(funObj,x0,options,varargin)
% minFunc(funObj,x0,options,varargin)
%
% Unconstrained optimizer using a line search strategy采用一个线性搜索策略优化无限制优化模型
%
...
%
% It computes descent directions using one of ('Method'):计算下降方向的方法
%   - 'sd': Steepest Descent
...
%           low-rank Hessian approximation)具有有限内存BFGS修正Quasi Newton（默认使用预定数前面的步骤形成一个低阶Hessian近似）
...
%
% Inputs:
%   funObj is a function handle函数句柄
%   x0 is a starting vector;开始的向量
%   options is a struct containing parameters结构体
...
%   MaxIter - Maximum number of iterations allowed (500)最大迭代数为500
